...
class AgenticOrchestrator:
    def __init__(self, mcp_server: MCPDataServer, api_key: str):
        ...
        self.fallback_model = "gemini-1.5-flash"  # A genuinely different model for meaningful fallback
    ...
    def _call_model(self, prompt: str, system_prompt: str = None) -> str:
        """Call Gemini model with fallback support for text generation"""
        ...
        try:
            ...
            try:
                # Also pass system_prompt on fallback so instruction context is not lost
...
What additional search terms should we try?"""
            ...
            plan = self._call_model(plan_prompt)
            if not plan:
                plan = "Search for policy details"
...
Be concise - just say YES (sufficient) or NO (need more searches)"""
            ...
            verification_text = self._call_model(verify_prompt)
            if not verification_text:
                verification_text = "YES"

            verification = verification_text.strip().upper()
...
Based on the above policy information and reasoning, provide a comprehensive answer. Always cite your sources and be precise."""
            ...
            answer = self._call_model(prompt, system_prompt=system_prompt)
            if not answer:
    ...
    def run(self, query: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if hasattr(self.mcp_server, 'search_engine') and hasattr(self.mcp_server.search_engine, 'get_cache_stats'):
                engine_cache = self.mcp_server.search_engine.get_cache_stats()
                self.monitor.cache_hits = engine_cache.get('hits', 0)
                self.monitor.cache_misses = engine_cache.get('misses', 0)
